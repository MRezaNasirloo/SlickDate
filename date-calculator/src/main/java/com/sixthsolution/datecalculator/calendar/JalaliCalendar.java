...
/**
 * <p>
 * {@link JalaliCalendar} is a subclass of Java built-in {@link Calendar},
 * and it implements all of its functionality.
 * </P>
 * <font color="red">
 * NOTE: Months start from 0 to 11
 * </font>
 */
public class JalaliCalendar extends Calendar {
  public static int gregorianDaysInMonth[] = {
      31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31
  };
  public static int jalaliDaysInMonth[] = {
  ...
  public JalaliCalendar(TimeZone zone, Locale aLocale) {
    ...
    yearMonthDay = gregorianToJalali(yearMonthDay);
  ...
  JalaliCalendar(int year, int month, int dayOfMonth, int hourOfDay, int minute, int second,
      int millis) {
    super();
    ...
    YearMonthDay yearMonthDay =
        jalaliToGregorian(new YearMonthDay(fields[1], fields[2], fields[5]));
    cal = new GregorianCalendar(yearMonthDay.getYear(), yearMonthDay.getMonth(),
        yearMonthDay.getDay(), hourOfDay, minute, second);
  ...
  public static YearMonthDay gregorianToJalali(YearMonthDay gregorian) {
    ...
    gregorianDayNo = 365 * gregorian.getYear() + (int) Math.floor((gregorian.getYear() + 3) / 4)
        - (int) Math.floor((gregorian.getYear() + 99) / 100) + (int) Math.floor(
        (gregorian.getYear() + 399) / 400);
  ...
  public static int weekOfYear(int dayOfYear, int year) {
    switch (dayOfWeek(JalaliCalendar.jalaliToGregorian(new YearMonthDay(year, 0, 1)))) {
      case 2:
        dayOfYear++;
        break;
      case 3:
  ...
  public static int dayOfWeek(YearMonthDay yearMonthDay) {

    Calendar cal = new GregorianCalendar(yearMonthDay.getYear(), yearMonthDay.getMonth(),
        yearMonthDay.getDay());
  ...
  @Override protected void computeTime() {
    ...
    if (!isTimeSet && !isTimeSeted) {
      ...
      YearMonthDay yearMonthDay = jalaliToGregorian(
    ...
    switch (field) {
      ...
      case DST_OFFSET: {
        if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR) && isSet(HOUR_OF_DAY) &&
          ...
          cal = new GregorianCalendar();
          YearMonthDay yearMonthDay = jalaliToGregorian(
              new YearMonthDay(internalGet(YEAR), internalGet(MONTH), internalGet(DATE)));
          cal.set(yearMonthDay.getYear(), yearMonthDay.getMonth(), yearMonthDay.getDay(),
              internalGet(HOUR_OF_DAY), internalGet(MINUTE), internalGet(SECOND));
          cal.set(field, value);
          yearMonthDay =
              gregorianToJalali(new YearMonthDay(cal.get(YEAR), cal.get(MONTH), cal.get(DATE)));
  ...
  @Override protected void computeFields() {
    ...
    if (!areFieldsSet) {
      ...
      super.set(DAY_OF_WEEK,
          dayOfWeek(jalaliToGregorian(new YearMonthDay(fields[1], fields[2], fields[5]))));
      //***
  ...
  @Override public void add(int field, int amount) {
    ...
    if (field == MONTH) {
      ...
      YearMonthDay yearMonthDay =
          jalaliToGregorian(new YearMonthDay(get(YEAR), get(MONTH), get(DATE)));
      Calendar gc = new GregorianCalendar(yearMonthDay.getYear(), yearMonthDay.getMonth(),
          yearMonthDay.getDay(), get(HOUR_OF_DAY), get(MINUTE), get(SECOND));
      gc.add(field, amount);
      yearMonthDay = gregorianToJalali(new YearMonthDay(gc.get(YEAR), gc.get(MONTH), gc.get(DATE)));
      super.set(YEAR, yearMonthDay.getYear());
      super.set(MONTH, yearMonthDay.getMonth());
      super.set(DATE, yearMonthDay.getDay());
  ...
  @Override public void roll(int field, int amount) {
    ...
    switch (field) {
      ...
      case WEEK_OF_YEAR: {//TODO: Implement this roll
        break;
      }
      case DAY_OF_YEAR: {
